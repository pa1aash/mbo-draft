...
That last sentence is a description of the interaction reported in the body, and the two measurements belong together. Premise validity here is a property of the surrogate--optimizer \emph{pair}, not of the surrogate: the same ensemble covers at $0.97$ on one optimizer's proposals and $0.41$ on another's. So the significant $\eta^2_{\text{inter}}$ of the main decomposition and this table are the same phenomenon measured two ways, one as variance attribution and one as a direct mechanistic reading of what the interaction consists of. Coverage figures are probabilities rather than min--max normalized scores, so the $0.41$-versus-$0.97$ contrast is untouched by the normalizer critique that bears on every $\eta^2$ in this paper. As stated in the body, the $0.97$ value belongs to the scikit-learn exact GP rather than the differentiable GP the grid scores; the repository carries both implementations.
...
\begin{table}[t]\centering
\caption{GP-minus-ensemble gap in raw oracle units (audited engine, X1 and X3 on, $\beta{=}2$, $K{=}5$, 30 seeds). GP is the better of the two GP classes per cell. Last column: perturbation gap as a percentage of the larger aggressive-optimizer gap.}
\label{tab:rawgap}
...
\midrule
Branin-2D      & $6.95$   & $0.05$  & $6.71$   & $0.8$ \\
Styblinski-5D  & $15.13$  & $6.00$  & $14.65$  & $39.6$ \\
Levy-8D        & $0.46$   & $0.05$  & $0.53$   & $8.8$ \\
Rosenbrock-10D & $0.14$   & $0.01$  & $0.15$   & $8.1$ \\
Rastrigin-15D  & $6.87$   & $0.23$  & $7.79$   & $3.0$ \\
Ackley-20D     & $5.10$   & $0.13$  & $5.32$   & $2.5$ \\
Griewank-30D   & $288.46$ & $25.56$ & $321.78$ & $7.9$ \\
\bottomrule
\end{tabular}
\end{table}
...
\paragraph{An independent reproduction of the headline scalar.} The $\beta$ sweep and the corner decomposition are separate runs at different seed counts, and they meet at one operating point. The $\beta$-sweep endpoint at $\beta{=}2$ (10 seeds) gives $\eta^2_{\text{surr}}=0.40636$; the headline corner (30 seeds) gives $0.40455$. These are two independent runs at the same operating point landing $0.0018$ apart, not one measurement reported at two precisions. For a scalar whose central challenge is that it is an operating-point artifact, that is worth stating explicitly rather than leaving a reader to assume rounding.
