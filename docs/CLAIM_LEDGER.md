...
where it is underpowered. It is distinct from `PROVISIONAL` (D07, D19), where a number is
...
> **The one gate 0-A.3 did *not* discharge.** BM1 matched budgets on the **seven synthetic
> tasks**. Design-Bench was not re-run under matching, so **D19's optimizer half stays
> PROVISIONAL**: η²_opt leads in all four DB corners and the budget confound there is still
> unremoved. Promoting D19 on the strength of a synthetic-only arm would be exactly the
> cross-suite over-reach §6 exists to avoid. Stated here so the discharge is not assumed.
...
| **D19** | "On Design-Bench the surrogate main effect is essentially zero in all four engine corners — η²_surrogate 0.001 to 0.032 across five tasks, with every point estimate at the floor of its own bootstrap interval — and the Friedman omnibus fails to reject at p = 0.19 to 0.76; we therefore report no detectable difference at this power, not equivalence." | η²_surr 0.001 / 0.032 / 0.002 / 0.018 by corner, CIs in `docs/POD_DB_SCALE.md` §6.1; Friedman p 0.760 / 0.263 / 0.225 / 0.190; 7-task with mujoco η²_surr 0.044–0.091; GFP-dropped (4 tasks) 0.001–0.021; `results/db_corners/db_analysis.json` | **Agarwal NeurIPS 2021 [22]** — "the lack of statistically significant results does not demonstrate the absence of effect"; **Demšar JMLR 7(1) 2006** — >10 datasets for Friedman; Benavoli JMLR 17(5) 2016 on mean-rank pool dependence | **"With N=7 you are below the threshold for the test you ran; you have shown no *detectable* difference at this power, not equivalence."** Plus Benavoli: "your mean-rank conclusions depend on an undisclosed pool" (our `analysis.task_norm` silently spans 11 cells, including undefined `ens_conformal:*` arms). | **Rule 4.** The word "equivalent" appears nowhere in §6. Agarwal + Demšar are cited *by us*, converting the objection into the framing. An explicit power/N specification is attached. The 11-cell pooling bug is disclosed and the analysis unified to 9 cells before any rank claim is made; per Benavoli's own recommendation, pairwise claims use tests that depend only on the two methods compared. | **PROVISIONAL** — the null is landed; the *optimizer* half **remains gated**. 0-A.3 matched budgets on the **seven synthetic tasks only**; Design-Bench was not re-run under matching, so η²_opt still leads in all four DB corners with D08's budget confound unremoved there. **Do not promote this row on the strength of a synthetic-only arm.** |
...
| **D19** | PROVISIONAL | PROVISIONAL | unchanged — DB not re-run under matching |
