...
Answer distribution: **yes 24 · partial 5 · no 2 · NA 1** (…of 31, 1 NA).
...
| 4.7 | Computing infrastructure (hardware **and** software) | **partial** *(upgraded 2026-07-26; see G1)* | **Now stated in the paper:** Supplement §Experimental Configuration ¶"Compute and environment." gives accelerator class (**CPU-only** — every artifact carries a `+cpu` PyTorch build, no GPU or CUDA anywhere), OS (x86-64 Linux 6.17.0-35-generic / glibc 2.39; the one TF-Bind-8 platform check on arm64 macOS 26.3.1), Python (3.12.3 synthetic / 3.9.23 Design-Bench), the full per-artifact version stamp, the parallelism model (process pool, one worker per core less one), and a throughput anchor (100 of 312 cells in 8.4 min on eight workers → the 1,890-cell synthetic grid is hours, not days). **Software/OS/commit are also enforced mechanically:** `REQUIRED_META` (`code/run_all.py:46-48`) and `load_checked` (`run_all.py:97-112`) reject any result file with an incomplete meta block; versions pinned in `requirements.txt`. **Still missing — why this stays *partial*:** 4.7 names *"GPU/CPU models; amount of memory"* explicitly, and neither the **CPU model** nor the **amount of RAM** is recorded in any artifact, log or doc, so neither can be stated without inventing it. 3 of 5 sub-items are fully satisfied (GPU: none; OS; library names+versions), 2 are not. |
...
|---|---|---|---|
| **G1** | **PARTIALLY CLOSED 2026-07-26.** Supplement §Experimental Configuration ¶"Compute and environment." now states accelerator class (CPU-only, no GPU/CUDA), OS and kernel, both Python environments, the per-artifact version stamp, the parallelism model, and a recorded throughput anchor. **Two facts remain unstated because no artifact, log or doc records them: the CPU model and the amount of RAM.** They are not derivable from this repository — the production runs were on a Linux box whose hardware was never stamped (`docs/AAAI27_VENUE.md:315` flagged exactly this; the fix that landed added timestamp/git-sha/versions but not hardware). | 4.7 (still *partial*) | **Author action, not a code fix:** supply CPU model, core count and RAM for the Linux box that produced the grids (and, optionally, a measured wall-clock for a full run — `run_all.py:329` prints it but never persists it). One sentence appended to the same paragraph then flips 4.7 to *yes*. **Before 07-28.** |
