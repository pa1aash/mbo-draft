...
| **G6** | **CLOSED 2026-07-28.** The blocker was not `newtx` but the **TeX Gyre Termes** fonts it depends on: `ts1-qtmr.tfm` (TS1-encoded Termes) was absent from this BasicTeX (`2026basic`) install, and AAAI's template needs TS1 for the U+2019 apostrophe on its line 214. Fixed with `tlmgr --usermode install tex-gyre` — a **user-tree** install (`~/Library/texmf`), no `sudo`, no system TeX modified, and **no change to the `.tex`**, which remains byte-faithful to AAAI's template apart from the 31 answer slots. | — | `ReproducibilityChecklist.pdf` now builds: 2 pages, letter, all 31 items rendered, PDF `author` field empty. |
