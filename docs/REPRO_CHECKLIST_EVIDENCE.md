...
| 4.7 | Computing infrastructure (hardware **and** software) | **partial** | **Software/OS/commit: complete and enforced.** Every artifact stamps `platform`, `os_release`, `python`, `torch`, `numpy`, `botorch`, `gpytorch`, `cma`, `git_sha` (`code/run_all.py:46-48`), and `code/run_all.py::load_checked` **refuses to load** any result file with an incomplete meta block (`run_all.py:97-112`). Versions pinned in `requirements.txt`. The paper states the stamping discipline as a contribution (§4 ¶"The protocol.": "Stamp the engine (library versions, flags, commit) with every number") and names one engine difference by consequence (§3: ARD RBF vs ARD Matérn-5/2 across pinned BoTorch versions). **Hardware: absent.** No CPU/GPU model, memory, or wall-clock appears anywhere in the paper. Gap **G1** — the single clearest checklist gap. |
...
|---|---|---|---|
| **G1** | **No hardware/compute description anywhere in the paper.** Software, OS and commit are stamped per-artifact and enforced, but CPU/GPU model, memory and wall-clock are stated nowhere. | 4.7 (*partial*) | Add 1–2 sentences to Supplement §Experimental Configuration: CPU model, core count, RAM, OS, and approximate wall-clock for the full grid. Costs no main-body page budget. **Do before 07-28.** |
